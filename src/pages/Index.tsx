...
import MarqueeGallery from "@/components/ui/MarqueeGallery";  // ⬅️ ใช้ตัวใหม่
...
const educations = [
  { id: 1, title: "Faculty of Science and Technology,Computer Science", institution: "Chiang Mai Rajabhat University", period: "2022 - Present" },
  { id: 2, title: "Science and Technology", institution: "Thungchang School", period: "2018- 2021" },
];

// Activities with captions
const activities = [
  { id: 1, src: activityImage1, alt: "ภาพประชุมทีม", caption: "Best Presenter Award – SCI-TECH CMRU Conference 2025" },
  { id: 2, src: activityImage2, alt: "ภาพบรรยายโครงการ", caption: "Presented at SCI-TECH CMRU Conference 2025" },
  { id: 3, src: activityImage3, alt: "รับรางวัล", caption: "ศึกษาการทำงานในหน่วยงานด้านนวัตกรรมดิจิทัล ณ ศูนย์บริหารจัดการเมืองอัจฉริยะ มหาวิทยาลัยเชียงใหม่" },
  { id: 4, src: activityImage4, alt: "กิจกรรมอื่น ๆ", caption: "ศึกษาการทำงานในสถานประกอบการด้านเทคโนโลยีตอมพิวเตอร์ อุทยานวิทยาศาสตร์และเทคโนโลยี มหาวิทยาลัยเชียงใหม่" },
  { id: 5, src: activityImage5, alt: "ภาพกิจกรรม 5", caption: "IoT Project: Smart Pet Feeding Device with Blynk IoT." },
];
...
const Index = () => {
  ...
  const NICKNAME = "แอล";
  const CURRENT_ADDRESS = "รอยัลแกรนด์ฮิลล์ ตำบลช้างเผือก อำเภอเมืองเชียงใหม่ เชียงใหม่ 50300";

  const DOB_ISO = "2003-11-12";
  const dob = new Date(DOB_ISO);
  const age = Math.floor((Date.now() - dob.getTime()) / (1000 * 60 * 60 * 24 * 365.2425));
  const dobText = dob.toLocaleDateString("th-TH", { day: "2-digit", month: "long", year: "numeric" });

  const neumorphismClass =
    "bg-[#E0E5EC] rounded-2xl p-6 shadow-[5px_5px_10px_#bebebe,-5px_-5px_10px_#ffffff]";

  const StaticHobbyCard: React.FC<{ icon: React.ComponentType<any>; label: string; }> = ({ icon: Icon, label }) => (
    <div className="group relative rounded-2xl p-[2px] bg-gradient-to-r from-indigo-400 via-fuchsia-400 to-emerald-400 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-300/20 cursor-default select-none">
      <span aria-hidden className="absolute inset-0 rounded-2xl blur-lg opacity-40 bg-gradient-to-r from-indigo-400 via-fuchsia-400 to-emerald-400" />
      ...
      <div className="flex flex-col lg:flex-row">
              ...
              <div className="text-center">
                ...
                <div className="space-y-3 mt-4 text-left">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-medium text-gray-800">{FULL_NAME_TH}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <User className="h-4 w-4 text-gray-400" />
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="ml-2 inline-flex items-center rounded-full bg-white/70 px-2 py-0.5 text-xs font-semibold text-gray-700 shadow-sm border border-gray-200">{age} ปี</span>
                  </div>
                  <div className="flex items-start gap-2 text-sm">
                    <MapPin className="h-4 w-4 text-gray-400 mt-0.5" />
                    <span className="text-gray-500 shrink-0">Current address:</span>
                    <span className="font-medium text-gray-800 break-words">{CURRENT_ADDRESS}</span>
                  </div>
                </div>
              </div>
        ...
        <main className="flex-1 w-full max-w-5xl mx-auto p-6 lg:p-8 min-w-0">
          <div className="space-y-8">
            ...
            <AnimatedSection>
              <section className={neumorphismClass}>
                ...
                <div className="relative border-l-2 border-gray-300 pl-6 space-y-8">
                  {educations.map((edu) => (
                    <ExperienceItem key={edu.id} title={edu.title} company={edu.institution} period={edu.period} />
                  ))}
                </div>
              </section>
            </AnimatedSection>

            {/* Activities – วิ่งอัตโนมัติแบบต่อเนื่อง + กดดูรูปใหญ่ได้ + มีคำบรรยาย */}
            <AnimatedSection>
              <section className={neumorphismClass}>
            ...
            {/* Certificates – วิ่งอัตโนมัติแบบต่อเนื่อง + กดดูรูปใหญ่ได้ + ไม่โชว์คำบรรยาย */}
            <AnimatedSection>
              <section className={neumorphismClass}>
                <div className="flex items-center gap-2 mb-6">
                ...
                <MarqueeGallery
                  ...
                  gap={24}
                  pauseOnHover={true}
                  showCaptions={false}   // ❗ ซ่อนคำบรรยาย
                />
              </section>
            </AnimatedSection>
